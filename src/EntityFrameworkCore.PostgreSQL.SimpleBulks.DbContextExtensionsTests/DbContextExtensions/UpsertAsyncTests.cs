...
using EntityFrameworkCore.PostgreSQL.SimpleBulks.Upsert;
...
{
    public UpsertAsyncTests(ITestOutputHelper output, PostgreSqlFixture fixture) : base(output, fixture, "BulkMergeTest")
    {
    }
    ...
    public async Task Upsert_Existing_Using_Linq_With_Transaction(int length)
    {
        ...
        dbRows = dbRows.OrderBy(x => x.Id).ToList();
        compositeKeyRows = compositeKeyRows.OrderBy(x => x.Id1).ThenBy(x => x.Id2).ToList();
        dbCompositeKeyRows = dbCompositeKeyRows.OrderBy(x => x.Id1).ThenBy(x => x.Id2).ToList();
