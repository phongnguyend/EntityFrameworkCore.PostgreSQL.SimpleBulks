...
public abstract class TableInfor<T> : TableInfor
{
    ...
    public abstract List<ParameterInfo> CreateNpgsqlParameters(NpgsqlCommand command, T data, IReadOnlyCollection<string> propertyNames, bool includeDiscriminator, bool autoAdd);
}
...
public class DbContextTableInfor<T> : TableInfor<T>
{
    ...
    public override List<ParameterInfo> CreateNpgsqlParameters(NpgsqlCommand command, T data, IReadOnlyCollection<string> propertyNames, bool includeDiscriminator, bool autoAdd)
    {
        ...
        if (includeDiscriminator && Discriminator != null && !propertyNames.Contains(Discriminator.PropertyName))
        {
            ...
            var para = (NpgsqlParameter)mapping.CreateParameter(command, CreateParameterName(Discriminator.PropertyName), Discriminator.PropertyValue ?? DBNull.Value);
...
public class NpgsqlTableInfor<T> : TableInfor<T>
{
    ...
    public override List<ParameterInfo> CreateNpgsqlParameters(NpgsqlCommand command, T data, IReadOnlyCollection<string> propertyNames, bool includeDiscriminator, bool autoAdd)
    {
        ...
        if (includeDiscriminator && Discriminator != null && !propertyNames.Contains(Discriminator.PropertyName))
        {
            var para = new NpgsqlParameter(CreateParameterName(Discriminator.PropertyName), Discriminator.PropertyValue ?? DBNull.Value);
